...
class Vigenere:
    ...
    def __init__(self, key=None):
        ...
        self.sym_to_idx = dict(
                    zip(self.symbols, range(len(self.symbols)))
                    )
        self.idx_to_sym = dict(
                    zip(range(len(self.symbols)), self.symbols)
                    )
    ...
    def encrypt(self, message: str) -> str:
        ...
        for split in split_message:
            ...
            for i in range(len(split)):
                new_index = (self.sym_to_idx[split[i]] + \
                            self.sym_to_idx[self.key[i]]) % len(self.symbols)
                # avoid having spaces in the cipher
                # substitute by '~'
                if self.idx_to_sym[new_index] == ' ':
                    chunk += '~'
                # avoid having new lines in the cipher
                # substitute by '`'
                elif self.idx_to_sym[new_index] == '\n':
                    chunk += '`'
                else:
                    chunk += self.idx_to_sym[new_index]
